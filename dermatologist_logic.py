...
#     temperature=0,
#     streaming=True,
#     api_key=os.environ.get("OPENROUTER_API_KEY"),  # set in .env
# )
...
You are a Senior Dermatological Assistant. Your goal is to extract and summarize medical information 
...
def diagnosis_retrieval_tool(query: str) -> str:
    """
    Retrieves comprehensive diagnostic information from the dermatology diagnosis vector store.
    Use for: Unconfirmed diseases, symptom identification, and differential diagnosis.
    """
...
def medication_retrieval_tool(query: str) -> str:
    """
    Retrieves pharmacological treatments, dosages, and protocols from the medication store.
    Use for: Confirmed diseases, drug names, and treatment safety.
    """
...
def search_tool(query: str) -> str:
    ...
    3. FINDING LOCAL DOCTORS: To find dermatology clinics, addresses, and contact details for patient referrals.
    """
    ...
    prompt = ChatPromptTemplate.from_template("""
        You are a Medical Data Refiner. Your job is to take raw search results and make them professional and clean.
...
def doctor_search_tool(location: str) -> str:
    """
    Search for and locate dermatologists and medical clinics near the patient.
    ...
    """
    final_query = f"dermatologists and skin clinics in {location}"
...
@tool
def analyze_skin_or_report_image(image_path: str) -> str:
    """
    Use this tool when a patient provides a photo of their skin condition or a medical report.
    ...
    if not os.path.exists(image_path):
        ...
        uploads_dir = os.path.join(base_dir, 'static', 'dermatalogist_upload')
    ...
    analysis_prompt = (
        "Check if this image is a medical report or a clinical photo. "
        "IF A REPORT: Transcribe all text, dates, and values exactly. "
        "IF A SKIN PHOTO: Describe the lesion's morphology, color, distribution, "
        "and any notable features like scaling or crusting."
    )
    try:
...
def generate_medical_report(
    ...
    """
    Generates a formal, professional dermatology consultation report in HTML/Tailwind format.
    ...
    Args:
        ...
        medications: List of objects with 'name', 'dosage', and 'duration'. Example: [{"name": "Drug", "dosage": "10mg", "duration": "7 days"}]
...
<div id="{report_id}" class="medical-report relative mx-auto flex min-h-[297mm] w-full max-w-[210mm] flex-col overflow-hidden bg-white font-sans text-slate-800 shadow-2xl sm:rounded-sm dark:bg-gray-950 dark:text-gray-200">
  ...
  <div class="relative w-full flex-shrink-0 bg-[#0ea5e9] py-8 sm:py-10">
    <div class="relative z-10 flex flex-col items-center justify-between gap-6 px-6 sm:flex-row sm:items-start sm:px-10">
      <div class="flex flex-col items-center gap-3 text-center sm:flex-row sm:text-left">
        ...
        <div class="text-white">
          <h1 class="text-xl font-black leading-none tracking-tight uppercase sm:text-2xl">Dermatology<br/><span class="text-sky-100">Clinic</span></h1>
          <p class="mt-1 text-[8px] font-bold tracking-[0.2em] text-sky-100 uppercase opacity-90">Consultation Report</p>
        </div>
      </div>
      <div class="text-center sm:text-right">
        <h2 class="text-xl font-bold leading-tight text-white sm:text-2xl uppercase tracking-tight">Dr. Zavian</h2>
        <p class="text-xs font-extrabold text-sky-100">Consultant Dermatologist</p>
        <p class="text-[9px] font-bold tracking-wide text-white/90">MBBS, FCPS (Dermatology)</p>
  ...
  <div class="relative z-10 -mt-6 px-4 sm:px-10">
    <div class="grid grid-cols-2 gap-y-3 gap-x-2 rounded-xl border border-slate-200 bg-white px-5 py-4 shadow-lg dark:border-gray-800 dark:bg-gray-900 sm:grid-cols-5 sm:gap-3 sm:px-6">
      ...
      <div class="col-span-2 sm:col-span-1 sm:text-right">
        ...
        <span class="font-mono text-[11px] font-bold text-[#0ea5e9]">{patient_id}</span>
      </div>
    </div>
  </div>
  ...
  <div class="relative z-10 flex flex-1 flex-col gap-6 px-6 pt-8 sm:flex-row sm:gap-8 sm:px-10">
    <div class="w-full space-y-6 sm:w-[35%] sm:border-r sm:border-slate-100 sm:pr-6 sm:dark:border-gray-800">
      <section>
        <h3 class="mb-2 text-[9px] font-black tracking-[0.15em] text-[#0ea5e9] uppercase">Complaints</h3>
      ...
      <section>
        <h3 class="mb-2 text-[9px] font-black tracking-[0.15em] text-[#0ea5e9] uppercase">Clinical Notes</h3>
    ...
    <div class="w-full sm:w-[65%]">
      ...
      <div class="mb-6 overflow-hidden rounded-xl border border-slate-200 shadow-sm dark:border-gray-800">
        <table class="w-full table-fixed border-collapse text-left">
          <thead>
            <tr class="bg-slate-900 text-[8px] font-black tracking-widest text-white uppercase dark:bg-sky-900">
      ...
      <div class="grid gap-3">
        ...
        <div class="rounded-xl border border-sky-100 bg-sky-50/40 p-3 dark:border-sky-900/30 dark:bg-sky-900/10">
          <h3 class="mb-1 text-[8px] font-black text-sky-700 uppercase dark:text-sky-500">Treatment Plan</h3>
      ...
      <div class="relative w-full text-center sm:w-56">
        <div class="absolute -top-10 left-1/2 -translate-x-1/2 -rotate-12 pointer-events-none opacity-50">
          <div class="h-20 w-20 rounded-full border-4 border-double border-red-600/50 flex flex-col items-center justify-center text-[6px] font-black text-red-600 uppercase">
            <span>Verified</span>
            <span class="text-[8px]">Dr. Zavian</span>
            <span>Clinic</span>
          </div>
        </div>
        <div class="border-t border-slate-800 pt-2">
          <p class="text-xs font-black text-slate-800 uppercase dark:text-gray-100">Dr. Zavian</p>
          <p class="text-[8px] font-bold tracking-[0.2em] text-[#0ea5e9] uppercase">Consultant Specialist</p>
        </div>
      </div>
    </div>
  </div>

  <div class="relative w-full bg-[#0ea5e9] py-3">
...
<div class="mx-auto mt-8 mb-12 flex max-w-[210mm] justify-center px-4 print:hidden">
  <button onclick="downloadReport('{report_id}')" class="group flex items-center gap-3 rounded-full bg-[#0ea5e9] px-8 py-3 text-xs font-black tracking-widest text-white shadow-lg transition-all hover:bg-[#0284c7] hover:shadow-xl active:scale-95">
...
async function downloadReport(reportId) {{
    ...
    // Ensure the clone is visible and has white/dark background
    ...
    try {{
        ...
        link.download = `Medical_Report_Dr_Zavian_${{new Date().toISOString().split('T')[0]}}.png`;
        link.href = canvas.toDataURL('image/png');
        link.click();

    }} catch (error) {{
        console.error("Report generation failed:", error);
        alert("Failed to generate report image. Please contact support.");
    }} finally {{
...
# Bind tools
tools = [diagnosis_retrieval_tool, medication_retrieval_tool, search_tool, generate_medical_report, doctor_search_tool, analyze_skin_or_report_image]
...
def model_calling(messages: List[BaseMessage]) -> AIMessage:
    ...
    "ROLE: You are Dr. Zavian, an expert Dermatologist. "
    ...
    "You must perform a deep clinical investigation. Ask about: duration, pain/itch levels, spread, "
    "triggers, previous treatments, and any other relevant symptoms. Aim for a complete clinical picture."
    "\n3. PHASE 3: TOOL CALL: Only after you have a full and detailed history, call 'diagnosis_retrieval_tool'."
    "\n4. PHASE 4: DIFFERENTIAL REFINEMENT: If the tool returns multiple possible diagnoses (e.g., Acne vs. Rosacea), "
    "you MUST NOT proceed to medication yet. Ask highly specific follow-up questions to distinguish between "
    "the options. Move deeper until you are sure of the specific condition."
    "\n5. PHASE 5: MEDICATION CALL: Only after you are sure and the patient asks for treatment/medication, "
    "call 'medication_retrieval_tool' passing the exact narrowed-down result. Do not suggest medicine from memory."
    "\n6. FALLBACK: Use internal knowledge ONLY if tools return 'No results found'."
    "\n7. IMAGE TRIGGER: If a patient uploads an image at ANY point, you MUST immediately call the 'analyze_skin_or_report_image' tool. DO NOT ask questions like 'Can you tell me more about this image?' before calling the tool. Analyze first, then talk."

    "\n\nPATIENT-FRIENDLY LANGUAGE: When explaining conditions or giving a diagnosis, "
    "NEVER use complex medical jargon. Use simple, everyday English."

"\n\nCRITICAL RULES FOR RESPONSE FORMAT (STRICT):"
    "\n1. SINGLE QUESTION RULE: You are strictly FORBIDDEN from asking multiple questions in a single response. "
    "If you need 4 pieces of info, you must ask 4 separate times. Never use 'and' to link two different questions."
    "\n2. NEVER provide multiple versions, variations, or options for a response."
    "\n3. Generate only ONE response. Stop immediately after the first complete thought or single question."
    "\n4. NEVER use long sequences of dots (....) or placeholders."
    "\n5. STABILITY RULE: Never generate repetitive or random strings. If repeating, STOP immediately."
  

    "\n\nCLINICAL RIGOR & REASONING: Medical accuracy is critical. NEVER make assumptions. "
    "Perform step-by-step clinical reasoning for every case."

    "\n\n--- REPORT INTEGRITY & GENERATION (STRICT NO-ASSUMPTION RULE) ---"
    "\n1. NO ASSUMPTIONS: You are strictly FORBIDDEN from assuming, inventing, or hallucinating any patient data "
    "(Name, Age, Symptoms, or Medications) for the medical report. If a data point is missing, you MUST ask the patient "
    "to provide it before calling the report tool."
    "\n2. MANDATORY: You are strictly FORBIDDEN from typing a medical report as text. Use the 'generate_medical_report' tool."
    "\n3. FORBIDDEN FORMAT: Never type 'Dermatology Consultation Report...'. This is a CRITICAL FAILURE."
    "\n4. ZERO TOLERANCE FOR MISSING DATA: Never pass 'N/A' or empty strings to the report tool. "
    "Re-verify demographics if they are missing from memory."
    "\n5. PATIENT ID: Generate a unique 8-digit alphanumeric ID (e.g., ZAV-123456)."
    "\n6. FIELD MAPPING: "
    "\n   - 'Complaints' & 'Clinical Notes': Professional and highly detailed based ONLY on facts gathered."
    "\n   - 'medications': List of objects with 'name', 'dosage', and 'duration' keys. Use data ONLY from 'medication_retrieval_tool'. "
    "\n   - 'Instructions': Numbered list of 5-6 actionable care steps."
    "\n7. POST-GENERATION: Say 'I have generated your detailed medical report. You can download it below.' and STOP."

    "\n\nCORE RULES FOR TOOLS:"
    "\n1. Never show raw tool output, JSON, or technical logs to the patient."
    "\n2. IMAGE ANALYSIS: Use 'analyze_skin_or_report_image'. Hide the Local Path."
))
