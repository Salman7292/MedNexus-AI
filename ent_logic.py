...
def get_medication_store_ent():
    from langchain_pinecone import PineconeVectorStore
    return PineconeVectorStore(
        index_name="ent-specialist",
        embedding=get_embeddings(),
        namespace="medication_knowledge"
    )

def get_diagnosis_store_ent():
    from langchain_pinecone import PineconeVectorStore
    return PineconeVectorStore(
        index_name="ent-specialist",
        embedding=get_embeddings(),
...
def get_geo_directory_store_ent():
    from langchain_pinecone import PineconeVectorStore
    return PineconeVectorStore(
        index_name="ent-specialist",
        embedding=get_embeddings(),
        namespace="ent_geo_directory"
    )
...
You are a Senior ENT Clinical Assistant. Your goal is to extract and summarize medical information 
from the provided context to answer a doctor's query accurately regarding ear, nose, and throat conditions.
...
def diagnosis_retrieval_tool(query: str) -> str:
    """
    Retrieves comprehensive diagnostic information from the ENT diagnosis vector store.
    Use for: Sinus issues, hearing loss, throat conditions, and differential diagnosis in otolaryngology.
    """
    retriever = get_diagnosis_store_ent().as_retriever(search_kwargs={"k": 5})
...
def medication_retrieval_tool(query: str) -> str:
    """
    Retrieves pharmacological treatments, dosages, and protocols from the ENT medication store.
    Use for: Antibiotics for ear infections, nasal sprays, throat treatments, and treatment safety.
    """
    retriever = get_medication_store_ent().as_retriever(search_kwargs={"k": 5})
    docs = retriever.invoke(query)
    return process_clinical_search(query, docs)
...
def search_tool(query: str) -> str:
    ...
    2. To check for the latest 2024-2026 FDA drug approvals or emerging ENT clinical guidelines.
       PRIORITY: Always search "https://dailymed.nlm.nih.gov/dailymed/index.cfm" first for medication details.
    3. FINDING LOCAL DOCTORS: To find ENT clinics, otolaryngologists, addresses, and contact details for patient referrals.
    """
   
    client = TavilyClient(api_key=os.environ.get("TAVILY_API_KEY"))
    # Prepend site filter if it looks like a drug search
    is_drug_search = any(term in query.lower() for term in ["drug", "medication", "dose", "tablet", "syrup", "spray", "drops", "fda"])
    ...
    prompt = ChatPromptTemplate.from_template("""
        You are a Medical Data Refiner for ENT. Your job is to take raw search results and make them professional and clean.
...
def doctor_search_tool(location: str) -> str:
    """
    Search for and locate ENT specialists and medical clinics near the patient.
    ...
    """
    final_query = f"ENT specialists in {location}"
    retriever = get_geo_directory_store_ent().as_retriever(
...
@tool
def analyze_ent_or_report_image(image_path: str) -> str:
    """
    Use this tool when a patient provides a photo of their throat, ear, or a medical report.
    ...
    if not os.path.exists(image_path):
        # Try to find the most recent image in static/dermatalogist_upload
        base_dir = os.path.dirname(os.path.abspath(__file__))
        uploads_dir = os.path.join(base_dir, 'static', 'ent_upload')
    ...
    analysis_prompt = (
        "Check if this image is a medical report or a medical photo related to ENT (ear, nose, throat). "
        "IF A REPORT: Transcribe or extract all text, dates, and values exactly. "
        "IF AN ENT PHOTO: Describe the findings in the ear, nose, or throat, "
        "noting any swelling, redness, discharge, or inflammation."
    )
    try:
...
def generate_medical_report(
    ...
    """
    Generates a formal, professional ENT consultation report in HTML/Tailwind format.
    ...
    Args:
        ...
        medications: List of objects with 'name', 'dosage', 'frequency', and 'duration'. Example: [{"name": "Drug", "dosage": "10mg", "frequency": "BID", "duration": "7 days"}]
    ...
    for m in medications:
        ...
        med_rows += f"""
        <tr class="group transition-colors hover:bg-cyan-50/30">
            <td class="px-3 py-2">
            ...
            <td class="px-3 py-2">
                <span class="rounded bg-cyan-100 px-1.5 py-0.5 text-[9px] font-black text-cyan-700 dark:bg-cyan-900/40 dark:text-cyan-300">
                    {m_freq}
                </span>
            </td>
    ...
    formatted_instructions = "".join([
        ...
        f'<div class="mr-1.5 font-black text-[#00acc1]">{i+1}.</div>'
        f'<div>{step}</div></div>' 
...
<div id="{report_id}" class="medical-report relative mx-auto flex min-h-[297mm] w-full max-w-[210mm] flex-col overflow-hidden bg-white font-sans text-slate-800 shadow-2xl sm:rounded-sm dark:bg-gray-950 dark:text-gray-200">
  ...
  <div class="relative w-full flex-shrink-0 bg-[#00acc1] py-8 sm:py-10">
    <div class="relative z-10 flex flex-col items-center justify-between gap-6 px-6 sm:flex-row sm:items-start sm:px-10">
      <div class="flex flex-col items-center gap-3 text-center sm:flex-row sm:text-left">
        <div class="flex h-16 w-16 items-center justify-center rounded-full border-4 border-white/30 bg-white shadow-lg">
          <img src="https://raw.githubusercontent.com/Salman7292/Agentic-AI/refs/heads/main/ai_doctor_logo.gif" alt="Clinic Logo" class="h-11 w-11 rounded-full object-contain" />
        </div>
        <div class="text-white">
          <h1 class="text-xl font-black leading-none tracking-tight uppercase sm:text-2xl">ENT Specialist<br/><span class="text-cyan-100">Clinic</span></h1>
          <p class="mt-1 text-[8px] font-bold tracking-[0.2em] text-cyan-100 uppercase opacity-90">Consultation Report</p>
        </div>
      </div>
      <div class="text-center sm:text-right">
        <h2 class="text-xl font-bold leading-tight text-white sm:text-2xl uppercase tracking-tight">Dr. Anees</h2>
        <p class="text-xs font-extrabold text-cyan-100">ENT Specialist</p>
        <p class="text-[9px] font-bold tracking-wide text-white/90">MBBS, FCPS (Otolaryngology)</p>
  ...
  <div class="relative z-10 -mt-6 px-4 sm:px-10">
    <div class="grid grid-cols-2 gap-y-3 gap-x-2 rounded-xl border border-slate-200 bg-white px-5 py-4 shadow-lg dark:border-gray-800 dark:bg-gray-900 sm:grid-cols-5 sm:gap-3 sm:px-6">
      ...
      <div class="col-span-2 sm:col-span-1 sm:text-right">
        ...
        <span class="font-mono text-[11px] font-bold text-[#00acc1]">{patient_id}</span>
      </div>
    </div>
  </div>
  ...
  <div class="relative z-10 flex flex-1 flex-col gap-6 px-6 pt-8 sm:flex-row sm:gap-8 sm:px-10">
    <div class="w-full space-y-6 sm:w-[35%] sm:border-r sm:border-slate-100 sm:pr-6 sm:dark:border-gray-800">
      <section>
        <h3 class="mb-2 text-[9px] font-black tracking-[0.15em] text-[#00acc1] uppercase">Complaints</h3>
      ...
      <section>
        <h3 class="mb-2 text-[9px] font-black tracking-[0.15em] text-[#00acc1] uppercase">Clinical Notes</h3>
        <div class="text-[10px] leading-snug text-slate-600 dark:text-gray-400">{notes}</div>
      </section>
      <section class="rounded-xl bg-cyan-50 p-4 dark:bg-cyan-950/30">
        <h3 class="mb-1 text-[8px] font-black tracking-widest text-cyan-600 uppercase">Diagnosis</h3>
    ...
    <div class="w-full sm:w-[65%]">
      ...
      <div class="mb-6 overflow-hidden rounded-xl border border-slate-200 shadow-sm dark:border-gray-800">
        <table class="w-full table-fixed border-collapse text-left">
          <thead>
            <tr class="bg-slate-900 text-[8px] font-black tracking-widest text-white uppercase dark:bg-cyan-900">
      ...
      <div class="grid gap-3">
        ...
        <div class="rounded-xl border border-cyan-100 bg-cyan-50/40 p-3 dark:border-cyan-900/30 dark:bg-cyan-900/10">
          <h3 class="mb-1 text-[8px] font-black text-cyan-700 uppercase dark:text-cyan-500">Treatment Plan</h3>
      ...
      <div class="relative w-full text-center sm:w-56">
        <div class="absolute -top-10 left-1/2 -translate-x-1/2 -rotate-12 pointer-events-none opacity-50">
          <div class="h-20 w-20 rounded-full border-4 border-double border-red-600/50 flex flex-col items-center justify-center text-[6px] font-black text-red-600 uppercase">
            <span>Verified</span>
            <span class="text-[8px]">Dr. Anees</span>
            <span>Clinic</span>
          </div>
        </div>
        <div class="border-t border-slate-800 pt-2">
          <p class="text-xs font-black text-slate-800 uppercase dark:text-gray-100">Dr. Anees</p>
          <p class="text-[8px] font-bold tracking-[0.2em] text-[#00acc1] uppercase">ENT Specialist</p>
        </div>
      </div>
    </div>
  </div>

  <div class="relative w-full bg-[#00acc1] py-3">
...
<div class="mx-auto mt-8 mb-12 flex max-w-[210mm] justify-center px-4 print:hidden">
  <button onclick="downloadReport('{report_id}')" class="group flex items-center gap-3 rounded-full bg-[#00acc1] px-8 py-3 text-xs font-black tracking-widest text-white shadow-lg transition-all hover:bg-[#0097a7] hover:shadow-xl active:scale-95">
...
async function downloadReport(reportId) {{
    ...
    try {{
        ...
        link.download = `ENT_Report_Dr_Anees_${{new Date().toISOString().split('T')[0]}}.png`;
...
# Bind tools
tools = [diagnosis_retrieval_tool, medication_retrieval_tool, search_tool, generate_medical_report, doctor_search_tool, analyze_ent_or_report_image]
...
def model_calling(messages: List[BaseMessage]) -> AIMessage:
    ...
    "ROLE: You are Dr. Anees, an expert Otolaryngologist (Ear, Nose, and Throat Specialist). "
    "STYLE: Short, WhatsApp-style (1-2 sentences). Direct and empathetic. "
    "GOAL: You are the treating physician. Do NOT refer the patient to another 'human doctor' or 'professional'—YOU are the professional. "
    ...
    "You must perform a deep clinical investigation for sinus, hearing, or throat issues. "
    "Ask about: duration, severity of pain/discomfort, spread, triggers, previous treatments, and any other relevant symptoms."
    "\n3. PHASE 3: THE DIFFERENTIAL LOOP (CRITICAL): If the 'diagnosis_retrieval_tool' returns multiple possibilities (e.g., Pharyngitis vs. Esophagitis):"
    "\n   - DO NOT suggest both. DO NOT tell the patient to see another doctor."
    "\n   - Pick the top 2-3 results and ask 'Differentiator Questions' (e.g., 'Is the pain worse with spicy food or when lying down?')."
    "\n   - Only proceed to Phase 4 once you have narrowed the symptoms to ONE specific condition."
    "\n4. PHASE 4: TOOL CALL: Only after you have a full and detailed history, call 'diagnosis_retrieval_tool'."
    "\n5. PHASE 5: MEDICATION CALL: After the diagnosis tool, you MUST immediately call 'medication_retrieval_tool' "
    "passing the exact results from the diagnosis tool to get the exact treatment plan. Do not suggest medicine from memory."
    "\n6. FALLBACK: Use internal knowledge ONLY if tools return 'No results found'."
    ...
    "\n\nEMPATHY EXAMPLE:"
    "\nPatient: 'I can't hear well, and it's frustrating.'"
    "\nDr. Anees: 'I completely understand how isolating hearing issues can feel. We’ll look into this together. Let me ask...'"

    "\n\nFEW-SHOT STYLE:"
    "\nPatient: 'I have a sore throat and it's hard to swallow.'"
    "\nDr. Anees: 'I'm sorry to hear you're in pain. Throat discomfort can be very taxing. You are in good hands.'"
    ...
    "until you have sufficient evidence to provide a safe and accurate diagnosis."
    ...
    "\n2. FORBIDDEN FORMAT: Never type a response like this: 'ENT Consultation Report... Patient Information... Complaints... Clinical Notes... Diagnosis... Medications...'. This type of plain-text reporting is a CRITICAL FAILURE."
    ...
    "\n4. PATIENT ID: Always generate a unique 8-digit alphanumeric ID (e.g., ENT-887766) for the 'patient_id' field."
    ...
    "\n3. IMAGE ANALYSIS: Use 'analyze_ent_or_report_image'. Extract the Local Path from [System Alert] but keep it hidden."
    "\n4. CONVERSION RULE: Always take the information from the tools and rephrase it into your own clinical voice (Dr. Anees)."
))
