...
function updateVoiceModalUI(state, text = '') {
    ...
    const visualizer = document.querySelector('.voice-visualizer-container'); // Need to ensure this exists or use a parent
...
function showWelcomeMessage() {
    ...
    typeText(titleEl, "Hello, I'm Dr. Zavian.", 40, () => {
        typeText(subtitleEl, "How can I help you regarding your skin health today?", 20);
    });
}
...
function clearImage() {
    ...
    if (sendBtn && (!userInput.value.trim().length > 0)) {
        ...
        sendBtn.classList.add('scale-0', 'opacity-100');
    }
}
    ...
    [
        ...
        voices.forEach(voice => {
            ...
            option.onclick = () => {
                ...
                // Update highlighting in both containers
...
async function loadThreads() {
    try {
        const response = await fetch('/threads?specialist=derma');
...
async function loadHistory(threadId) {
    ...
    try {
        const response = await fetch(`/history/${threadId}?specialist=derma`);
...
function showTypingIndicator() {
    ...
    let headerHtml = `
        <div class="flex items-center gap-2 mb-1 pl-1">
             <span class="text-xs font-bold bg-clip-text text-transparent bg-gradient-to-r from-brand-600 to-purple-600 dark:from-brand-400 dark:to-purple-400 uppercase tracking-wider">Dr. Zavian</span>
             <span class="px-1.5 py-0.5 rounded text-[10px] bg-gray-100 dark:bg-gray-800 text-gray-500 font-medium">Dermatologist</span>
        </div>
    `;
    ...
    bubble.innerHTML = `
        <div class="flex gap-4 w-full">
           <div class="shrink-0 flex flex-col items-center">
                <img src="/static/dr_salman.png" alt="Dr. Zavian" class="w-10 h-10 rounded-full object-cover ring-2 ring-white dark:ring-gray-900 shadow-md">
            </div>
...
function appendMessage(role, content, animate = true, index = null) {
    ...
    bubble.className = isUser
        ? "px-4 py-2.5 text-white text-base font-normal leading-relaxed tracking-gemini rounded-2xl rounded-br-sm bg-gradient-to-br from-blue-500 to-cyan-500 dark:from-blue-600 dark:to-cyan-600 shadow-md translate-y-[-10px]"
...
async function sendMessage() {
    ...
    userInput.value = "";
    if (window.resetInputHeight) window.resetInputHeight();
    ...
    formData.append('specialist', 'derma');
    if (selectedImage) {
...
function showConversationMenu(event, threadId, anchorEl) {
    ...
    menu.className = 'fixed bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl shadow-2xl py-1 w-48 z-[100] animate-in fade-in zoom-in duration-200';
    ...
    menu.innerHTML = `
        <button onclick="openMediaVaultFromMenu('${threadId}')" class="w-full flex items-center gap-3 px-3 py-2.5 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors first:rounded-t-xl last:rounded-b-xl">
...
// New function to handle opening media vault from the context menu
function openMediaVaultFromMenu(threadId) {
    // Close the context menu first
    const menu = document.getElementById('conversation-context-menu');
    if (menu) menu.remove();

    // Open the media vault with the thread ID
    openMediaVault(threadId);
}

// Make sure openMediaVault is properly defined
async function openMediaVault(threadId) {
    if (!threadId) {
        console.error("No thread ID provided for media vault");
        return;
    }
    ...
    try {
        ...
        if (data.messages) {
            data.messages.forEach((msg, idx) => {
                ...
                if (typeof msg.content === 'object' && msg.content && msg.content.image) {
                    mediaItems.push({
...
function openLightbox(index) {
    if (!window.currentMediaItems || index < 0 || index >= window.currentMediaItems.length) return;
    window.currentMediaIndex = index;
    const item = window.currentMediaItems[index];
    ...
    if (typeLabel) typeLabel.textContent = item.type.toUpperCase();
    if (dateLabel && item.date) {
        ...
        dateLabel.textContent = 'Consultation Item';
    }
...
function navigateLightbox(direction) {
    ...
    if (newIndex >= 0 && newIndex < window.currentMediaItems.length) {
        openLightbox(newIndex);
    }
}

// Add download functionality for lightbox
document.addEventListener('DOMContentLoaded', function () {
    const downloadBtn = document.getElementById('download-media-btn');
    if (downloadBtn) {
        downloadBtn.addEventListener('click', async () => {
            const item = window.currentMediaItems?.[window.currentMediaIndex];
            if (!item) return;
