...
function openVoiceMode() {
    ...
    voiceModeModal.classList.remove('hidden', 'opacity-0', 'pointer-events-none', 'translate-y-10');
    voiceModeModal.classList.add('flex', 'opacity-100', 'active');
...
function closeVoiceMode() {
    ...
    voiceModeModal.classList.add('hidden', 'opacity-0', 'pointer-events-none');
    voiceModeModal.classList.remove('flex', 'opacity-100', 'active');
...
function updateVoiceModalUI(state, text = '') {
    ...
    voiceStatusText.className = 'text-brand-500 dark:text-brand-400 text-[10px] font-medium uppercase tracking-tighter';

    switch (state) {
        case 'listening':
            voiceStatusText.textContent = 'Listening...';
            voiceStatusText.classList.remove('text-brand-500', 'dark:text-brand-400');
            voiceStatusText.classList.add('text-green-500', 'dark:text-green-400');
        ...
        case 'thinking':
            ...
            // Base classes are already set
            orbListeningBg?.classList.add('hidden');
            if (modalAiFeedback) {
                modalAiFeedback.textContent = 'Dr. Anees is thinking...';
        ...
        case 'speaking':
            ...
            // Base classes are already set
            if (modalAiFeedback) {
                modalAiFeedback.textContent = 'Dr. Anees is speaking...';
                modalAiFeedback.classList.remove('opacity-0');
            }
            voiceOrb?.classList.add('scale-110', 'shadow-[0_0_120px_rgba(14,165,233,0.6)]');
            // Show stop button
            modalStopBtn?.classList.remove('hidden');
            modalRightSpacer?.classList.add('hidden');
            break;
        default:
            ...
            // Base classes are already set
            orbListeningBg?.classList.add('hidden');
            voiceOrb?.classList.remove('scale-105', 'scale-110', 'shadow-[0_0_100px_rgba(34,197,94,0.4)]', 'shadow-[0_0_120px_rgba(14,165,233,0.6)]');
            // Hide stop button
...
document.getElementById('mobile-voice-mode-trigger')?.addEventListener('click', () => {
    ...
    if (window.innerWidth < 1024 && typeof toggleSidebarMobile === 'function') {
        toggleSidebarMobile();
    }
    openVoiceMode();
});
...
if (modalTypeTrigger) {
    modalTypeTrigger.addEventListener('click', () => {
        closeVoiceMode();
    });
}
...
if (userInput) {
    ...
    // Reset height helper for use after sending
...
function showWelcomeMessage() {
    const welcomeHtml = `
        <div id="welcome-message" class="flex flex-col items-center justify-center mt-4 md:mt-8 text-center space-y-4">
            <img src="/static/ent_logo.gif" alt="ENT Specialist Logo" class="w-32 h-32 object-contain rounded-full shadow-lg dark:shadow-none bg-white dark:bg-transparent p-1 animate-fade-in">
            <h1 id="welcome-title" class="text-2xl md:text-3xl font-medium text-transparent bg-clip-text bg-gradient-to-r from-brand-600 to-purple-600 dark:from-brand-200 dark:to-white min-h-[2.5rem]">
            </h1>
    ...
    typeText(titleEl, "Hello, I'm Dr. Anees.", 40, () => {
        typeText(subtitleEl, "How can I help you regarding your ear, nose, or throat health today?", 20);
    });
}
...
function handleImageSelect(event) {
    ...
    reader.onload = function (e) {
        ...
        // Modal specific preview
        if (modalImagePreview) modalImagePreview.src = e.target.result;
        if (modalImagePreviewContainer) modalImagePreviewContainer.classList.remove('hidden');
    };
    reader.readAsDataURL(file);

    // Show send button when image is selected
    if (sendBtn) {
...
function stopVoiceInput() {
    ...
    if (voiceBtn) voiceBtn.classList.remove('text-brand-500', 'dark:text-brand-400');
    // Also reset mobile voice button if visible
    const mobileMicBtn = document.getElementById('voice-btn-mobile');
    if (mobileMicBtn) mobileMicBtn.classList.remove('text-brand-500', 'dark:text-brand-400');
...
function populateVoiceList() {
    ...
    voices.forEach(voice => {
        ...
        option.onclick = () => {
            ...
            // Highlight selected
            Array.from(voiceSelectContainer.children).forEach(child => {
                child.classList.remove('bg-brand-50/50', 'dark:bg-brand-900/20', 'text-brand-600', 'dark:text-brand-400');
            });
            option.classList.add('bg-brand-50/50', 'dark:bg-brand-900/20', 'text-brand-600', 'dark:text-brand-400');
        };
...
function initVoiceSelect() {
    if (!voiceDropdownBtn || !voiceDropdownMenu) return;
    ...
    document.addEventListener('click', (e) => {
        if (!voiceDropdownBtn.contains(e.target) && !voiceDropdownMenu.contains(e.target)) {
...
async function loadThreads() {
    try {
        const response = await fetch('/threads?specialist=ent');
...
async function loadHistory(threadId, silent = false) {
    currentThreadId = threadId;

    // Clear Chat UI only if not silent
    if (!silent) {
        if (chatMessagesArea) chatMessagesArea.innerHTML = ''; else chatContainer.innerHTML = '';
        // Show loading state if not silent
        const loadingHtml = '<div class="text-center text-gray-400 mt-10 italic">Syncing session...</div>';
        if (chatMessagesArea) chatMessagesArea.innerHTML = loadingHtml; else chatContainer.innerHTML = loadingHtml;
    }

    try {
        const response = await fetch(`/history/${threadId}?specialist=ent`);
        const data = await response.json();

        if (data.messages && data.messages.length > 0) {
            // If silent, we just update the data-index of existing elements instead of re-rendering everything
            // But for simplicity and correctness of order, we re-render only if there's a difference or just overwrite
            // To fix the "extra loading" flicker, we only re-render if NOT silent.
            // If silent, it's usually called after a chat to sync indices.

            if (silent) {
                // Update message indices in DOM without clearing
                const messageElements = (chatMessagesArea || chatContainer).querySelectorAll('[data-index]');
                data.messages.forEach((msg, idx) => {
                    if (messageElements[idx]) {
                        messageElements[idx].setAttribute('data-index', msg.index);
                    }
                });
            } else {
                if (chatMessagesArea) chatMessagesArea.innerHTML = ''; else chatContainer.innerHTML = '';
            ...
            // If no messages and not silent, show welcome
            showWelcomeMessage();
        }
...
function showTypingIndicator() {
    ...
    const headerHtml = `
        <div class="flex items-center gap-2 mb-1 pl-1">
             <span class="text-xs font-bold bg-clip-text text-transparent bg-gradient-to-r from-brand-600 to-purple-600 dark:from-brand-400 dark:to-purple-400 uppercase tracking-wider">Dr. Anees</span>
             <span class="px-1.5 py-0.5 rounded text-[10px] bg-gray-100 dark:bg-gray-800 text-gray-500 font-medium">ENT Specialist</span>
        </div>
    `;
    ...
    bubble.innerHTML = `
        <div class="flex gap-4 w-full">
           <div class="shrink-0 flex flex-col items-center">
                <img src="/static/images/ent2.png" alt="Dr. Anees" class="w-10 h-10 rounded-full object-cover ring-2 ring-white dark:ring-gray-900 shadow-md">
            </div>
    ...
    if (typingSound) {
        typingSound.volume = 0.15; // Set volume to 15%
        typingSound.currentTime = 0; // Reset
        const playPromise = typingSound.play();
        if (playPromise !== undefined) {
            playPromise.catch(e => console.log('Typing sound blocked:', e.message));
        }
    }
}
...
// Redundant voice mode logic removed
...
async function downloadReport(reportId) {
    ...
    try {
        ...
        link.download = `Medical_Report_Dr_Anees_${new Date().toISOString().split('T')[0]}.png`;
...
function isHtmlReport(content) {
    return typeof content === 'string' &&
        (content.includes('medical-report') ||
            content.includes('report-container') ||
            (content.includes('Dr. Zavian') && content.includes('Consultant Dermatologist') && content.includes('<!-- Header -->')) ||
            (content.includes('Dr. Anees') && content.includes('ENT Specialist')));
}
...
function appendMessage(role, content, animate = true, index = null) {
    ...
    const headerHtml = isUser ? '' : `
        <div class="flex items-center gap-2 mb-1 pl-1">
             <span class="text-xs font-bold bg-clip-text text-transparent bg-gradient-to-r from-brand-600 to-teal-600 dark:from-brand-400 dark:to-teal-400 uppercase tracking-wider">Dr. Anees</span>
             <span class="px-1.5 py-0.5 rounded text-[10px] bg-gray-100 dark:bg-gray-800 text-gray-500 font-medium">ENT Specialist</span>
        </div>
    `;
    ...
    bubble.className = isUser
        ? "px-4 py-2.5 text-white text-base font-normal leading-relaxed tracking-gemini rounded-2xl rounded-br-sm bg-gradient-to-br from-brand-500 to-teal-500 dark:from-brand-600 dark:to-teal-600 shadow-md transform translate-y-[-8px] relative"
...
async function sendMessage() {
    ...
    if (isVoiceModeActive) {
        updateVoiceModalUI('thinking', text);
    }
    ...
    formData.append('specialist', 'ent');
    if (selectedImage) {
    ...
    try {
        ...
        const isAutoSpeakEnabled = (autoSpeakToggle && autoSpeakToggle.checked) || (mobileAutoSpeakToggle && mobileAutoSpeakToggle.checked);
        if (firstTokenReceived && aiContentDiv && isAutoSpeakEnabled) {
        ...
        // Refresh history to ensure indices are loaded for editing (silent mode)
        if (firstTokenReceived) {
            loadHistory(currentThreadId, true);
        }
    } catch (err) {
        ...
        if (userInput) userInput.disabled = false; // Always re-enable input
        if (sendBtn && (userInput.value.trim().length > 0 || selectedImage)) {
            sendBtn.classList.remove('scale-0', 'opacity-0');
            sendBtn.classList.add('scale-100', 'opacity-100');
        }
    }
...
function isHtmlReport(text) {
    return text.includes('clip-path-header') || text.includes('medical-report');
}
...
function showConversationMenu(event, threadId, anchorEl) {
    ...
    menu.innerHTML = `
        <button onclick="openMediaVault('${threadId}')" class="w-full flex items-center gap-3 px-3 py-2.5 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800/50 transition-colors first:rounded-t-xl last:rounded-b-xl">
            <i class="fa-solid fa-photo-film text-brand-500 dark:text-brand-400"></i>
